...
class ColumnBuilder<RecordType> {
	...
	public AddSortNum = (pageNumber: number, pageSize: number, width?: string | number) => {
		const col: IColumnProps<RecordType> = {
			...
			title: '序号',
			width,
			render: (value: any, record: RecordType, index: number) => (pageNumber - 1) * pageSize + index + 1
		}
	...
	// 将布尔值映射为是/否
	public addBoolean = (title: string, filed: string, width?: any, trueValue = '是', falseValue = '否') => {
		const column: IColumnProps<RecordType> = {
			dataIndex: filed,
			title,
			width,
	...
	// 创建格式化的日期列
	public addDate = (title: string, filed: string, format: string = 'YYYY-MM-DD', width?: string | number) => {
		const column: IColumnProps<RecordType> = {
			dataIndex: filed,
			title,
			width,
	...
	public addCodeIdToName = (title: string, filed: string, codeTableName: GroupName, width?: string | number) => {
		const column: IColumnProps<RecordType> = {
			dataIndex: filed,
			title,
			width,
	...
	public addConstantIdsToName = (title: string, filed: string, codeTableName: GroupName, width?: string | number) => {
		const column: IColumnProps<RecordType> = {
			dataIndex: filed,
			title,
			width,
